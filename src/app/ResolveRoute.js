...
const reg = pattern => {
    pattern = pattern
        ...
        .replace(
            '<account-tab>',
            '(blog|posts|comments|replies|payout|followed|followers|settings|notifications|communities|transfers)'
        )
        .replace(
            '<sort>',
...
export default function resolveRoute(path) {
    ...
    if (path === '/') return { page: 'HomePage' };
    if (path === '/home') return { page: 'HomePage' };
    // static
